    ...
    def build(self,n=2):
        ...
        for i in range(len(switches)):
            for j in range(i+1,len(switches)):
                self.addLink(switches[i],switches[j],bw=BANDWIDTH, delay=DELAY, loss=LOSS, max_queue_size=MAX_QUEUE_SIZE)

def simpleTest():
    "Create and test a simple network"
    topo = MeshTopo(n=10)
    net = Mininet(topo=topo,controller = RemoteController, host=CPULimitedHost, link=TCLink ,waitConnected=True)
    net.start()
